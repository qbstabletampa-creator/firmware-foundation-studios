...
**RESOLVED 2026-06-10 (Stable):** went with a better version of (B): App Store Connect API key (`~/.apple-keys/AuthKey_7ZF97YMSGA.p8` + EXPO_ASC_* + EXPO_APPLE_TEAM_* in `~/.env.integrations`) — app-specific passwords CANNOT create signing credentials, the ASC key can. First-time credential prompts driven via the PTY rig at `C:/Users/rodge/.eas-pty/driver.js`; credentials now live on EAS servers so all future builds are fully headless non-interactive.
**Xcode pin DISPROVEN 2026-06-10:** the Xcode-15.4 image pin FAILS for SDK 54 apps (React Native requires Xcode >= 16.1; pod install dies; 3 paid builds burned). Pins removed from light-snake/noah/manna eas.json — EAS default image is correct. Gosple (SDK 56) still carries the pin; remove before its next build. Never pin an image without a successful build proving it — pre-flight pod install for free with the `ios-preflight` GitHub Action (`gh workflow run ios-preflight -f app=<dir>`).
