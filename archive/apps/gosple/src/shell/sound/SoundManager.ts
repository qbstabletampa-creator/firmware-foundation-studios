...
// play() rewinds to the start (seekTo 0) then plays, so rapid repeats (key
// mashing) restart cleanly instead of being dropped.
//
...
//
// The 6 file names are the shared FFS contract (see .pmloop/sound-spec.md).
// Gosple is a word puzzle, so game.tsx maps its events onto these names:
//   tap      → keyboard key tap (quiet)
//   thorn    → invalid / wrong (soft uh-oh): bad word shake + loss
//   catch    → subtle tile/correct ding (win row reveal)
//   levelup  → win celebration chime
//   verse    → verse reveal chime
//   gameover → gentle game-over (kept distinct from loss "thorn")
// ---------------------------------------------------------------------------
...
// the tap is quietest so menu/keyboard navigation never feels loud, the chimes
// sit a touch higher so they read as rewards.
...
function play(name: SoundName): void {
  ...
  if (!player) return;
  try {
    // Rewind so repeated triggers (fast key taps) always restart.
    player.seekTo(0);
    player.play();
  } catch {
