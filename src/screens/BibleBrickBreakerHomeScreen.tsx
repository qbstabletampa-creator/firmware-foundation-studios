...
import styles from './BibleBrickBreakerHomeScreen.module.css';
...
export function BibleBrickBreakerHomeScreen() {
  ...
  const lastPlayedDate = useBibleBrickBreakerStore((s) => s.lastPlayedDate);

  const purchased = usePurchaseStore((s) => s.bibleBrickBreaker);
  const freeUsed = usePurchaseStore(
    (s) => s.bibleBrickBreakerLastFreeDate === new Date().toISOString().slice(0, 10),
  );
    ...
    <ScreenShell showTabs={false}>
      <div className={styles.container}>
        ...
        >
          <header className={styles.header}>
            <motion.button
              ...
              onClick={() => navigate('/bible-brick-breaker')}
              aria-label="Back"
              whileTap={{ scale: 0.95 }}
            >
              &#x2190;
            ...
            >
              <span className={styles.logoEmoji}>{'\u{1F9F1}'}</span>
            </motion.div>
            <motion.button
              className={styles.settingsBtn}
              onClick={() => navigate('/bible-brick-breaker/settings')}
              whileTap={{ scale: 0.95 }}
            >
              &#x2699;
            </motion.button>
          ...
          <h1 className={styles.gameTitle}>Bible Brick Breaker</h1>
          <p className={styles.tagline}>Break through to God's Word</p>
        </motion.div>
        ...
        >
          ...
          <div className={styles.statsRow}>
            ...
            <div className={styles.statItem}>
              ...
              <span className={styles.statValue}>
                {playedToday ? 'Today' : lastPlayedDate || '—'}
              </span>
            </div>
          </div>
        ...
        <motion.button
          ...
          onClick={() => navigate('/bible-brick-breaker/play')}
